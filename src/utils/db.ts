      ...
      if (isLoggedIn && evtRes.data && evtRes.data.length === 0) {
        ...
        try {
          await Promise.all([
            ...
            supabase.from('coupons').upsert(DEFAULT_COUPONS.map(mapCoupon.toDb)).then(({ error }) => { if (error) console.warn('Could not seed coupons on cloud:', error); })
          ]);
        } catch (seedErr) {
  ...
  resetDatabase(): void {
    ...
    if (client) {
      ...
      Promise.all([
        ...
        client.from('coupons').delete().neq('id', 'placeholder').then(({ error }) => { if (error) console.warn('Could not clear coupons on cloud:', error); })
      ]).then(() => {
        Promise.all([
          ...
          client.from('coupons').upsert(DEFAULT_COUPONS.map(mapCoupon.toDb)).then(({ error }) => { if (error) console.warn('Could not seed coupons on cloud:', error); })
